...
export async function main(ns) {
	...
	// 	if (ns.corporation.createCorporation('ACME', false)) {
	// 		ColorPrint('lightgreen', 'Created corporation using government money');
	// 	}
	// 	else if (ns.corporation.createCorporation('ACME', true)) {
	...
	// 	//ns.corporation.expandIndustry('Agriculture', 'ACME Farms');
	// 	corp = ns.corporation.getCorporation();
...
async function UpgradeOfficesAndHire(ns, division, mainCity, targetEmployeeCount, mainCount, dryRun = true) {
	...
	for (const city of CITIES) {
		...
		try {
			...
			let cost = ns.corporation.getExpandCityCost() + ns.corporation.getPurchaseWarehouseCost();
		...
		if (missing > 0) {
			...
			let cost = ns.corporation.getOfficeSizeUpgradeCost(division, city, missing);
			totalCost += cost;
			...
			if (!dryRun) {
				...
				ns.corporation.upgradeOfficeSize(division, city, missing);
				...
				// ns.corporation.getEmployee(division, city, employee)
				// ns.corporation.assignJob(division, city, employee, job)
...
async function HireEmployees(ns, division, city) {
	...
	for (let i = 0; i < missing; i++) {
		let emp = ns.corporation.hireEmployee(division, city);
...
async function DistributeEmployees(ns, division, city, mainCity) {
	...
	for (let empName of office.employees) {
		let emp = ns.corporation.getEmployee(division, city, empName);
		...
		if (emp.pos == 'Unassigned') {
			...
			await ns.corporation.assignJob(division, city, emp.name, newJob);
...
async function GetStorageInfo(ns, division) {
	...
	for (let city of CITIES) {
		try {
			let wh = ns.corporation.getWarehouse(division.name, city);
			size += wh.size;
...
async function SetSales(ns, division, saleMode) {
	...
	for (let city of CITIES) {
		...
		for (let i = 0; i < matProducts.length; i++) {
			switch (saleMode) {
				case SALE_MODES.FREEZE:
					ns.corporation.sellMaterial(division.name, city, matProducts[i], '0', 'MP*10000', false);
					break;
				case SALE_MODES.NORMAL:
					ns.corporation.sellMaterial(division.name, city, matProducts[i], 'MAX', 'MP*1.0775', false);
					break;
				case SALE_MODES.EMPTY:
					ns.corporation.sellMaterial(division.name, city, matProducts[i], 'MAX', 'MP', false);
					break;
				case SALE_MODES.FRAUD:
					ns.corporation.sellMaterial(division.name, city, matProducts[i], 'MAX', 'MP*1.057', false);
					break;
			}
		}
		...
		// 	switch (saleMode) {
		// 		case SALE_MODES.FREEZE:
		// 			ns.corporation.sellProduct(division.name, city, products[i], '0', 'MP*1000', false);
		// 			break;
		// 		case SALE_MODES.NORMAL:
		// 			ns.corporation.sellProduct(division.name, city, products[i], 'MAX', 'MP*0.75', false);
		// 			break;
		// 		case SALE_MODES.EMPTY:
		// 			ns.corporation.sellProduct(division.name, city, products[i], 'MAX', 'MP*0.75', false);
		// 			break;
		// 		case SALE_MODES.FRAUD:
		// 			ns.corporation.sellProduct(division.name, city, products[i], 'MAX', 'MP', false);
		// 			break;
		// 	}
		// }
	}
